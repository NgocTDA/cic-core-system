...
const SendBalanceFormPage: React.FC = () => {
  ...
  // State phục vụ cho popup Nhập Excel
  const [importModalVisible, setImportModalVisible] = useState(false);
  const [importMaDauMoi, setImportMaDauMoi] = useState('31358001');
  const [importPhanLoai, setImportPhanLoai] = useState('D40');
  const [importNgayBaoCao, setImportNgayBaoCao] = useState<dayjs.Dayjs | null>(dayjs('31/08/2025', 'DD/MM/YYYY'));
  const [importTenTep, setImportTenTep] = useState('');
  ...
  // Tự động cập nhật tên tệp trong popup nhập Excel khi thông tin chung thay đổi
  useEffect(() => {
    if (importModalVisible) {
      const ext = importFile ? (importFile.name.split('.').pop()?.toUpperCase() || 'XLSX') : 'XLSX';
      const generatedName = getAutoGeneratedFileName(importPhanLoai, importMaDauMoi, importNgayBaoCao, ext);
      setImportTenTep(generatedName);
    }
  }, [importPhanLoai, importNgayBaoCao, importMaDauMoi, importFile, importModalVisible]);

  const handleOpenImportModal = () => {
    setImportMaDauMoi(uploadMaDauMoi);
    setImportPhanLoai(uploadPhanLoai);
    setImportNgayBaoCao(uploadNgayBaoCao);
    setImportTenTep(uploadTenTep);
    setImportFile(null);
  ...
  const beforeUploadExcel = (file: any) => {
    ...
    const generatedName = getAutoGeneratedFileName(importPhanLoai, importMaDauMoi, importNgayBaoCao, upperExt);
    setImportTenTep(generatedName);
    return false; // Chặn upload thực tế lên server
  };

  const handleConfirmImport = () => {
    if (!importFile) {
      message.error('Vui lòng kéo thả hoặc chọn một tệp Excel để nhập!');
      return;
    }

    if (!importTenTep.trim()) {
      message.error('Vui lòng nhập hoặc chọn Tên tệp!');
      return;
    }

    const validationError = validateFileName(importTenTep, importPhanLoai, importMaDauMoi, importNgayBaoCao);
    if (validationError) {
    ...
    const executeImport = () => {
      ...
      setTimeout(() => {
        // Đồng bộ thông tin chung sang form chính
        setUploadMaDauMoi(importMaDauMoi);
        setUploadPhanLoai(importPhanLoai);
        setUploadNgayBaoCao(importNgayBaoCao);
        setUploadTenTep(importTenTep);

        // Tạo báo cáo tạm thời để sinh dữ liệu đối soát giả lập chất lượng cao
        const mockReport: BalanceReport = {
          key: `temp_import_${Date.now()}`,
          stt: 1,
          ngayBaoCao: importNgayBaoCao ? importNgayBaoCao.format('DD/MM/YYYY') : '',
          ngayGui: '-',
          tenTep: importTenTep,
          phanLoaiTep: importPhanLoai,
          moTaTep: `Báo cáo cân đối thông tin tín dụng loại ${importPhanLoai}`,
          trangThai: 'TAO_MOI',
          maDauMoi: importMaDauMoi
        };

        const resultRows = generateTreeReconciliationData([mockReport]);
        const parentRow = resultRows.find(item => item.isParent);
        const importedDetails = parentRow?.children || [];

        // Cập nhật key duy nhất cho form chính và gán metadata
        const mappedDetails = importedDetails.map((child, index) => ({
          ...child,
          key: `edit_child_excel_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 5)}`,
          tenTep: importTenTep,
          parentKey: '',
        ...
        message.success(`Đã nhập dữ liệu thành công từ tệp Excel: ${importTenTep} (${mappedDetails.length} dòng dữ liệu đối soát)`);
      }, 800);
    };

    // Nếu kỳ báo cáo hoặc bảng chi tiết hiện tại đã có dữ liệu, hiển thị cảnh báo xác nhận
    if (editDetails.length > 0) {
      let confirmModal: any = null;
      confirmModal = Modal.confirm({
        title: 'Xác nhận ghi đè dữ liệu',
        content: 'Kỳ báo cáo hiện tại hoặc bảng chi tiết đã có dữ liệu. Việc nhập tệp Excel mới sẽ xóa toàn bộ số liệu hiện tại và ghi đè bằng dữ liệu trong tệp Excel. Bạn có chắc chắn muốn tiếp tục?',
        footer: (
  ...
  const getModalFilteredFileNames = () => {
    const dateStr = importNgayBaoCao ? importNgayBaoCao.format('DD/MM/YYYY') : '';
    return data
      .filter(item =>
        item.maDauMoi === importMaDauMoi &&
        item.phanLoaiTep === importPhanLoai &&
  ...
  const validateFileName = (
    fileName: string,
    loaiTep: string = uploadPhanLoai,
    maDauMoi: string = uploadMaDauMoi,
    ngayBaoCao: dayjs.Dayjs | null = uploadNgayBaoCao
  ): string | null => {
    ...
    const expectedPrefix = `${loaiTep}${maDauMoi}${ngayBaoCao ? ngayBaoCao.format('YYYYMMDD') : ''}`;
  ...
  return (
    <PageLayout noPadding>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '12px 0 8px' }}>
          {/* Metadata Grid */}
          <div style={{
            display: 'grid',
            ...
            <div>
              ...
              <Input value={importMaDauMoi} disabled style={{ width: '100%', height: 36 }} />
            </div>
            ...
            <div>
              ...
              <Select
                value={importPhanLoai}
                onChange={setImportPhanLoai}
                style={{ width: '100%' }}
                size="middle"
              >
                <Select.Option value="D10">D10 — Thông tin định danh khách hàng vay phát sinh</Select.Option>
            ...
            <div>
              ...
              <DatePicker
                value={importNgayBaoCao}
                onChange={setImportNgayBaoCao}
                format="DD/MM/YYYY"
                style={{ width: '100%', height: 36 }}
                disabledDate={(current) => {
                  return current && current.isAfter(dayjs().add(1, 'month').endOf('month'), 'day');
            ...
            <div>
              ...
              <AutoComplete
                value={importTenTep}
                onChange={setImportTenTep}
                options={getModalFilteredFileNames()}
                placeholder="Nhập hoặc chọn tên tệp..."
                style={{ width: '100%' }}
                filterOption={(inputValue, option) =>
                  option?.value.toUpperCase().indexOf(inputValue.toUpperCase()) !== -1
          ...
          <div style={{ marginTop: 8 }}>
            <Text strong style={{ fontSize: 12, display: 'block', marginBottom: 6 }}>
              Tải lên tệp Excel dữ liệu chi tiết
            </Text>
            {importFile ? (
              <div style={{
                padding: '12px 16px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13, color: '#14532d' }}>{importFile.name}</div>
                    <div style={{ fontSize: 11, color: '#166534' }}>
                      {(importFile.size / (1024 * 1024)).toFixed(2)} MB / 10 MB
                    </div>
                  </div>
                </div>
                <Button 
              ...
              >
                <p style={{ fontSize: 32, color: '#16a34a', margin: '0 0 8px' }}>
                  <CloudUploadOutlined />
                </p>
                <p style={{ fontSize: 14, fontWeight: 500, color: '#1f2937', margin: '0 0 4px' }}>
                  Kéo thả file Excel vào đây hoặc click để tải lên
                </p>
